...
function DashboardPage({ user, onLogout }) {
  return (
    <section className="mx-auto mt-10 w-[min(1160px,min(94%,100vw-1.5rem))] max-w-full space-y-5 pb-[max(1rem,env(safe-area-inset-bottom,0px))]">
      <div className="rounded-3xl border border-tech-line bg-tech-card p-6">
        ...
        <button
          type="button"
          onClick={onLogout}
          className="mt-4 min-h-[44px] rounded-full border border-tech-line bg-tech-bg/60 px-5 py-2.5 text-sm hover:text-tech-magenta"
        >
          Logout
        </button>
      </div>
      ...
      <div className="grid gap-5 md:grid-cols-2">
        <div className="rounded-3xl border border-tech-line bg-tech-card p-6">
          ...
          <ul className="mt-4 space-y-3">
            {eventList.map((event) => (
              <li key={event.id} className="rounded-xl border border-tech-line bg-tech-bg/50 p-3">
        ...
        <div className="rounded-3xl border border-tech-line bg-tech-card p-6">
          ...
          <ul className="mt-4 space-y-3">
            {documents.map((doc) => (
              <li key={doc.id} className="rounded-xl border border-tech-line bg-tech-bg/50 p-3">
